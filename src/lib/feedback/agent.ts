 ...
 *
 * @remarks
 * User messages may include attachments (screenshots, images).
...
export interface AgentMessage {
  ...
  content: string
  /** Optional file attachments (screenshots, images, PDFs) */
  attachments?: Attachment[]
}

/**
 * Result of running the feedback agent
 *
 * @remarks
 * Contains the AI response, token usage, cost, and any tool calls made.
 * Tool calls are logged for debugging and auditing.
 */
export interface AgentResult {
  /** Final text response from the AI */
  content: string
  /** Total input tokens used (includes system prompt and history) */
  inputTokens: number
  /** Total output tokens generated */
  outputTokens: number
  /** Total cost in USD */
  cost: number
  /** Tools invoked during response generation */
  toolCalls: ToolCall[]
}
 ...
 *
 * @remarks
 * Events are emitted during streaming to update the UI:
...
export interface StreamEvent {
  ...
  toolName?: string
  /** Tool input parameters (for 'tool_start') */
  toolInput?: Record<string, unknown>
  /** Tool result string (for 'tool_end') */
  toolResult?: string
  /** Token usage (for 'done' events) */
  usage?: { inputTokens: number; outputTokens: number }
  /** Total cost in USD (for 'done' events) */
  cost?: number
...
/**
 * Run the feedback agent with streaming response
 *
 * @remarks
 * This is the main entry point for the feedback chat AI. It:
 * - Supports multi-turn tool use (Claude can call multiple tools in sequence)
 * - Handles image/screenshot attachments via Claude's vision capability
 * - Streams text responses for real-time UI updates
 * - Calculates and reports token usage and cost
 *
 * The agentic loop continues until Claude provides a final text response
 * (no more tool calls) or MAX_TOOL_ITERATIONS is reached.
 *
 * @param messages - Conversation history with optional attachments
 * @param onEvent - Callback for streaming events (text, tool_start, tool_end, done, error)
 * @returns Complete result with response content, usage, and tool calls
 * @throws Error if API call fails
 *
 * @example
 * ```typescript
 * const result = await runFeedbackAgent(
 *   [{ role: 'user', content: 'Search for LED downlights' }],
 *   (event) => {
 *     if (event.type === 'text') {
 *       // Stream text to UI
...
/**
 * Run the feedback agent without streaming
 *
 * @remarks
